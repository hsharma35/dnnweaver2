...
static int test_dma(char *devicename, off_t addr, uint32_t size, uint32_t offset, uint32_t count, char *filename);
...
int main(int argc, char* argv[])
{
  ...
  printf("device = %s, address = 0x%016lx, size = 0x%08x, offset = 0x%08x, count = %u\n", device, address, size, offset, count);
