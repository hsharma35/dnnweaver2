...
int test_dma(char *device_name, int size, int count)
{
  ...
  if (perf.clock_cycle_count && perf.data_cycle_count) {
    printf("(data duty cycle = %lld%%)\n", (long long)perf.data_cycle_count * 100 / (long long)perf.clock_cycle_count);
    double gBypesPerSec = (perf.transfer_size) / (double) perf.clock_cycle_count;
    printf (" data rate %f bytes length = %d, rate = %f \n", gBypesPerSec, perf.transfer_size, (double)(long long)perf.data_cycle_count/(long long)perf.clock_cycle_count);
  }
