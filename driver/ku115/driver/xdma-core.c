...
static int engine_initialize(struct xdma_dev *lro, int interrupts_offset)
{
	...
	u32 w;
	int rc = 0;
	printk(KERN_WARNING "Read register at BAR %d, address 0x%p.\n", lro->config_bar_idx, reg);
	w = read_register(reg + 0x00);
	/* not a write nor a read engine found? */
	if (((w & 0x00ffff00UL) != 0x00c10000UL) && ((w & 0x00ffff00UL) != 0x00c20000UL)) {
		printk(KERN_WARNING "Engine identifier not found (found 0x%08x expected 0xC100/0xC200).\n", w);
		rc = -1;
		goto fail_identifier;
	}
	printk(KERN_WARNING "Engine identifier found 0x%08x with version %u.\n", w, w & 0xffU);
	...
	w = 0x0UL;
	printk(KERN_WARNING "Set engine controller enable mask: 0x%08x.\n", w);
...
void engine_reinit(const struct xdma_engine *engine)
{
	printk(KERN_WARNING "%s: Re-init DMA Engine %s\n", DRV_NAME, engine->name);
...
static int xdma_config(struct xdma_dev *lro, unsigned int config_offset)
{
	...
	/* XDMA Target Bridge */
	if (w == 0xFF01UL)
		printk(KERN_WARNING "XDMA Target Bridge\n");
	/* XDMA XDMA */
	else if (w == 0xFF02UL) {
		...
		/* engine capabilities in the inspector since version 2 */
			...
			printk(KERN_WARNING "WDMA = 0x%08x\n", w);
			if (w & CONFIG_WDMA_EN) {
				...
				printk(KERN_WARNING "Write Engine requires %d byte alignment.\n", lro->align[/*dir_to_dev=*/0]);
			}
			/* inspect read engine capabilities */
			w = read_register(reg + 0x20);
			printk(KERN_WARNING "RDMA = 0x%08x\n", w);
			if (w & CONFIG_RDMA_EN) {
				...
				printk(KERN_WARNING "Read Engine requires %d byte alignment.\n", lro->align[/*dir_to_dev=*/0]);
			}
		}
		/* any engine present? */
		if (lro->capabilities & (CAP_ENGINE_READ | CAP_ENGINE_WRITE)) {
			printk(KERN_WARNING "XDMA Scatter-Gather%s%s\n",
				(lro->capabilities & CAP_ENGINE_READ)?" Read":"",
				(lro->capabilities & CAP_ENGINE_WRITE)?" Write":"");
		} else {
			printk(KERN_WARNING "XDMA Target Bridge (Scatter-Gather Engines disabled in SOPC).\n");
		}
	}
	else
		printk(KERN_WARNING "XDMA System ID = 0x%04x.\n", w);
	w = read_register(reg + 0x04);
	/* bus, device and function */
	printk(KERN_WARNING "bus:dev.fn = %02x:%02x.%1x, payload = %d bytes, maxread = %d bytes\n",
...
static int probe_scan_for_msi(struct xdma_dev *lro, struct pci_dev *pdev)
{
	...
	if (msi_msix_capable(pdev, PCI_CAP_ID_MSIX)) {
		...
		if (rv < 0)
			dbg_init("Couldn't enable MSI-X mode: rv = %d\n", rv);
		...
		if (rv < 0)
			dbg_init("Couldn't enable MSI mode: rv = %d\n", rv);
		lro->msi_enabled = 1;
	} else {
...
static int probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	clear_contr = dev_get_drvdata(&pdev->dev);
        if (clear_contr)
                printk(KERN_WARNING "%s: Found stashed clearing bitstream from previously loaded driver %p\n",
	...
	/* SD_Accel */
	#if SD_ACCEL && 0
	printk(KERN_WARNING "xdma_config()\n");
	...
	if (enable_credit_mp & stream ) {
		printk(KERN_WARNING "Design in Steaming mode enable Credit feature \n");
		...
		struct xdma_engine *engine, enum chardev_type type)
{
	...
	if (lro->major == 0) {
		...
		if(i == MAX_XDMA_DEVICES){
			printk(KERN_WARNING "Device limit reached\n");
			goto fail_alloc;
		}
		dev_present[i] = 1;
		lro->instance = i;
